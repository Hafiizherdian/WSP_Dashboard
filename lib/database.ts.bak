...
// Map<year, Map<week, Map<product, UnitAgg>>>
// Dibentuk satu kali dari weekProductMap, menggantikan semua filter/reduce berulang
// di generateQuarterlyData dan generateL4WC4WData.
function buildByWeekMap(
  weekProductMap: Map<string, UnitAgg>,
): Map<number, Map<number, Map<string, UnitAgg>>> {
  // year → week → product → UnitAgg
...
async function processSalesRecords(filters?: FetchFilters): Promise<SalesData> {
  ...
  // ── OPTIMASI v7: byWeekMap dibuat satu kali, dipakai oleh quarterly & l4w ──
  // Menggantikan reconstructVirtualRecords yang mengalokasikan array besar.
  // byWeekMap: year → week → product → UnitAgg  (akses O(1) di mana-mana)
...
// ─── generateQuarterlyData — refactored (v7) ─────────────────────────────────
// Sebelum: menerima any[], filter O(N×52) berulang per kuartal dan per minggu.
// Sesudah: menerima Map pre-grouped, semua akses O(1).
async function generateQuarterlyData(
  ...
  // ── generateQuarterlyProductDetails — refactored ──────────────────────────
  // Sebelum: iterasi array records dua kali (per kuartal, per minggu).
  // Sesudah: iterasi weekMap yang sudah di-slice per kuartal (akses O(1)).
  const generateQuarterlyProductDetails = (
    quarterIndex: number,
    quarterWeeks: number[], // daftar week yang masuk kuartal ini dan ada di data
  ): QuarterlyProductDetail[] => {
    const qNum = quarterIndex + 1;

    // product → aggregated actuals untuk kuartal ini
    const productActualMap = new Map<string, {
      units_dos: number; units_bks: number; units_slop: number; units_bal: number;
    }>();

    // product → week → per-week actuals
    ...
    // Fallback: jika kuartal kosong, isi dengan produk yang ada di tahun ini
  ...
  // ── Debug log (sama seperti versi asli) ───────────────────────────────────
  ...
  for (let qIndex = 0; qIndex < quarters.length; qIndex++) {
    ...
    // Daftar minggu yang benar-benar ada di data untuk kuartal ini
    const quarterWeeks: number[] = [];
    for (let w = startWeek; w <= endWeek; w++) {
      if (yearWeekMap.has(w)) quarterWeeks.push(w);
    }

    // Hitung actual per kuartal: sum dari semua produk di semua minggu kuartal
    // — O(quarterWeeks × products), bukan O(N × 52)
    ...
    // ── weeklyBreakdown — akses O(1) dari yearWeekMap ─────────────────────
    const weeklyBreakdown: WeeklyBreakdown[] = [];
    for (let week = startWeek; week <= endWeek; week++) {
      const prodMap = yearWeekMap.get(week); // undefined jika minggu tidak ada di data

      // Sum seluruh produk untuk minggu ini — O(products), bukan O(N)
...
// Sebelum: menerima any[] (virtual records), iterasi ulang semuanya.
// Sesudah: iterasi langsung weekProductMap via byWeekMap.
function generateL4WC4WData(
  ...
  // Pilih yearWeekMap; jika tidak ada tahun spesifik, gabungkan semua
  ...
  // Build weeklyTotals dari Map langsung — O(weeks × products), tanpa array virtual
...
// Sebelum: menerima any[], iterasi ulang seluruh records.
// Sesudah: menerima Map<week, Map<product, UnitAgg>>, akses langsung.
function generateProductL4WC1WData(
  ...
  // Semua minggu yang relevan (l4w + c1w)
  ...
  relevantWeeks.forEach(week => {
    ...
    prodMap.forEach((agg, product) => {
      // Ambil omzet dari omzetByProductWeek menggunakan key yang sama seperti saat diisi
